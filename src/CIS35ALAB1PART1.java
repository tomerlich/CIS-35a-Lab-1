...
public class CIS35ALAB1PART1 {
	...
	// before we can output the data for the user we need to get our inputs from the
	// user for this we will use a scanner
	...
	public static void printSchedule(NumberFormat currencyFormat, double monthlyPayment) {
		...
		System.out.printf("\n\nMonthly Payment:%s\nTotal Payment:%s", currencyFormat.format(monthlyPayment),
		...
		for (int paymentNum = 0; paymentNum < loanTimeMonths; paymentNum++) {
			...
			System.out.printf("%-8d\t%-8s\t%-9s\t%-7s\n", paymentNum + 1, currencyFormat.format(interestAmount),
					currencyFormat.format(principleAmount), currencyFormat.format(acountBalance));
		}
	...
	public static void main(String[] Args) {
		...
		// We will use these formatting tools to automate showing dollar values as well
		// as percentages
		NumberFormat currencyFormat = NumberFormat.getCurrencyInstance();

		// we will call another method to gather data so that we can leave main
		// uncluttered
		gatherData();
		...
		// Now we will call a method to output all of our results in a nicely formatted
		// way
		// this will take two number formats and all of the data.
		printSchedule(currencyFormat, monthlyPaymentVal);

		// to prevent memory leaks we have to close the scanner other variables will get
